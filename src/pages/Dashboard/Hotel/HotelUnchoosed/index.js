...
export default function HotelUnchoosed() {
  ...
  useEffect(async () => {
    try {
      ...
      if (ticket.TicketType?.isRemote || !ticket.TicketType?.includesHotel) setTicket('noHotel');
      else if (ticket.TicketType?.includesHotel) setTicket('hotel');
    } catch (e) {
      setTicket('noTicket');
    }
  ...
  const options = {
    ...
    noTicket: (
      <StyledForbiddenMessage>
        ...
        pagamento antes de fazer a escolha de hospedagem
      </StyledForbiddenMessage>
    ),
    noHotel: (
      <StyledForbiddenMessage>
        ...
        <br />
        Prossiga para a escolha de atividade
      </StyledForbiddenMessage>
    ),
    hotel: (
      <StyledAllowedView>
        ...
        <HotelsContainer>
          {hotelsList.map((value) => (
            <HotelData data={value} roomsState={roomsState} selectedState={selectedState} key={value.id} />
          ))}
        </HotelsContainer>
        <RoomContainer>
          {roomsState[0].map((value) => (
            <Room selectedRoomState={selectedRoomState} data={value} booked={value.id === bookedRoom} key={value.id} />
          ))}
        </RoomContainer>
